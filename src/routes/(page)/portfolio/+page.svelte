...
<svelte:head>
	<title>{seo.portfolio.title} | {profile.name}</title>
	<meta name="description" content={seo.portfolio.description} />
</svelte:head>
...
<div class="grid grid-cols-1 gap-4 pt-8 md:grid-cols-2 lg:grid-cols-3">
	...
	{:then { items }}
		{#each items as project}
			...
			>
				<a href={url}>
					<img class="rounded-t" src={project.thumbnail} alt={project.title} />
				</a>
				<div class="p-5">
					<a href={url}>
